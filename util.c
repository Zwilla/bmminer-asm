...
bool extract_sockaddr(char *url, char **sockaddr_url, char **sockaddr_port)
{
    ...
    *sockaddr_url = url;
    ...
    if (url_end) {
        ...
        port_len = (int) (strlen(url_begin) - url_len - 1);
        if (port_len < 1)
            return false;
        port_start = url_end + 1;
    } else
        url_len = (int) strlen(url_begin);
...
static void recalloc_sock(struct pool *pool, size_t len)
{
    ...
    old = (size_t) strlen(pool->sockbuf);
    news = old + len + 1;
...
bool parse_method(struct pool *pool, char *s)
{
    ...
    if (!strncasecmp(buf, "mining.set_extranonce", 21)) {
        ...
        goto out_decref;if (!strncasecmp(buf, "client.reconnect", 16)) {
...
void _cg_memcpy(void *dest, const void *src, int n, const char *file, const char *func, const int line)
{
