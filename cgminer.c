...
OPT_WITH_ARG("--bitmain-dev",
			...
			"Set bitmain device (default: /dev/axi_fpga_dev)"),
...
OPT_WITHOUT_ARG("--compact",
        ...
        OPT_WITHOUT_ARG("--extranonce-subscribe",
                        			...
                        			"Enable 'extranonce' stratum subscribe for NiceHash by Zwilla"),
...
static char *opt_verusage_and_exit(const char *extra)
{
    printf("%s\nBuilt with "
                   ...
                   "mining support by Miguel Padilla.\n"
            , packagename);
...
static void curses_print_status(void)
{
	...
	cg_mvwprintw(statuswin, 0, 0, " " PACKAGE " CGminer version " VERSION " - Started perfect: %s", datestamp);
	...
	wclrtoeol(statuswin);
	cg_mvwprintw(statuswin, 5, 0, " Block: %s...  Diff:%s  Started CGminer: %s  Best share: %s   ",
...
{
    const unsigned int WATCHDOG_SICK_COUNT = sicktime / opt_watchdog;
    const unsigned int WATCHDOG_DEAD_COUNT = WATCHDOG_DEAD_TIME / opt_watchdog;
    //const unsigned int interval = WATCHDOG_INTERVAL;
    const unsigned int interval = opt_watchdog;
...
void print_summary(void)
{
    ...
    applog(LOG_WARNING, "Started CGminer at %s", datestamp);
    if (total_pools == 1)
...
int main(int argc, char *argv[])
{
    ...
    applog(LOG_WARNING, "Started CGminer %s", packagename);
    if (cnfbuf) {
